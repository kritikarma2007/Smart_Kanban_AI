...
export default function Register() {
  ...
  // Modern Asynchronous Handle Submit connecting to your backend
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // Real database registration successful!
        localStorage.setItem('smart_kanban_token', data.token);
        localStorage.setItem('smart_kanban_user', data.name);
        
        // Triggers UI updates across your app
        window.dispatchEvent(new Event('authChange'));
        navigate('/dashboard');
      } else {
        // Server returned an error (e.g., "User already exists with this email")
        alert(data.message || 'Registration failed');
      }
    } catch (error) {
      console.error('Error connecting to backend:', error);
      alert('Could not connect to server. Please check if your backend terminal is running!');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      >
        <div className="text-center mb-8">
          <h2 className="text-3xl font-extrabold text-white tracking-tight">
            Create your <span className="bg-gradient-to-r from-[#7c3aed] to-[#8a2be2] bg-clip-text text-transparent">Account</span>
          </h2>
          <p className="mt-2 text-sm text-gray-400">
            Or{' '}
            <Link to="/login" className="font-medium text-[#7c3aed] hover:text-[#8a2be2] transition-colors">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`block w-full pl-10 pr-3 py-2 bg-black/30 border ${
                    errors.name ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500/20' : 'border-white/10 focus:border-[#7c3aed] focus:ring-[#7c3aed]/20'
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className={`block w-full pl-10 pr-3 py-2 bg-black/30 border ${
                    errors.confirmPassword ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500/20' : 'border-white/10 focus:border-[#7c3aed] focus:ring-[#7c3aed]/20'
                  } rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-4 transition-all`}
                />
              </div>
